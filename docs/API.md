...
| `GET /api/projects` | `{ projects: [{ slug, name, webUrl, createdAt, defaultBranch, previewUrl, repo: { url, host, owner, repo, prototypePath } }] }`. `slug` is `<owner>-<repo>`; `name` is `repo`, or `owner/repo` when another project has the same repo name; `prototypePath` is `null` until the mirror has been read; `previewUrl` points at the default branch's screenshot (null when no Chromium, no prototype or no mirror yet) |
...
| `GET /api/projects/:slug/preview/:sha.png` | screenshot (370×722) of the prototype directory at that commit, rendered with headless Chromium on first request from a `git archive` export and cached at `previews/<slug>/<sha>.png` (immutable cache headers). `404` without Chromium, without an `index.html` at that commit, or after a failed render (not retried in this process). Refs responses carry `previewUrl` per branch/tag; cached shas no longer referenced are pruned after each refs listing |
