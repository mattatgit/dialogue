...
- localhost-only Node server (`server.js`) with Node-builtins-only modules `server/git.js`, `server/projects.js`, `server/deploy-key.js`, `server/preview.js`, `server/terminal.js`, `server/watch.js`
...
- prototype previews: project cards and branch tiles show real screenshots (`GET /api/projects/:slug/preview/:sha.png`, headless Chromium from `DIALOGUE_CHROMIUM`/`PUPPETEER_EXECUTABLE_PATH`/PATH, cached per commit under `.dialogue-data/previews/`); the static `assets/landline-proto.png` is gone. Without Chromium the initials badge / blank device is shown
